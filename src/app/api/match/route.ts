...
export async function GET(req: Request) {
  ...
  for (const buyer of buyers as Buyer[]) {
    for (const property of properties as Property[]) {
      // Skip if same agent (agent matching own buyer to own property — still useful)
      const score = computeScore(buyer, property)
      if (score < 60) continue

      // Upsert match (ignore if already exists)
      ...
      if (existing?.notified_at) continue // already notified
          ...
          html: `
            <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
              ...
              <div style="background:white;padding:24px;border:1px solid #eee;border-top:none;border-radius:0 0 12px 12px">
                ...
                <p style="margin-top:24px;color:#666;font-size:13px">Connectez-vous à votre dashboard Habiteo pour voir ce match et le marquer comme traité.</p>
              </div>
            </div>`,
        })
      }

      // Email to seller's agent (only for off-market, and only if different agent)
          ...
          html: `
            <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
              ...
              <div style="background:white;padding:24px;border:1px solid #eee;border-top:none;border-radius:0 0 12px 12px">
                ...
                <p style="color:#444;font-size:14px">Un confrère Habiteo a un acheteur dont le profil correspond à <strong>${score}%</strong> à votre bien off-market. Connectez-vous à votre dashboard pour voir les détails et décider si vous souhaitez entrer en contact.</p>
                <p style="margin-top:24px;color:#666;font-size:13px">Les coordonnées de l'acheteur ne sont pas transmises — tout passe par Habiteo.</p>
              </div>
            </div>`,
        })
      }
  ...
  return Response.json({ matched: newMatches, buyers: buyers.length, properties: properties.length })
}
